From Undecidability.L Require Export Datatypes.LVector.
From Complexity.L.Datatypes Require Import LNat Lists LFinType.

Require Import Undecidability.Shared.Libs.PSL.Vectors.Vectors.

#[global]
Instance termT_to_list X `{encodable X} n : computableTime' (Vector.to_list (A:=X) (n:=n)) (fun _ _ => (1,tt)).
Proof.
...
Import Vector.
#[global]
Instance termT_vector_map X Y `{encodable X} `{encodable Y} n (f:X->Y) fT:
  computableTime' f fT ->
...
Global
Instance termT_map2 n A B C `{encodable A} `{encodable B} `{encodable C} (g:A -> B -> C) gT:
...
#[global]
Instance termT_vector_eqb X `{encodable X} (n' m:nat) (eqb:X->X->bool) eqbT:
...
Global Instance eqbComp_Vector X `{encodable X} `{eqbCompT X (R:=_)} n:
  eqbCompT (Vector.t X n).
Proof.
  evar (c:nat). exists c. edestruct termT_vector_eqb with (X:=X). apply compT_eqb.
  eexists.
