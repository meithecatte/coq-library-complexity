From Undecidability.L.Functions Require Export FinTypeLookup.
From Complexity.L.Datatypes Require Import LFinType LBool LProd Lists.
...
Section Lookup.
  ...
  Global Instance termT_lookup `{encodable Y}:
    computableTime' (lookup (X:=X) (Y:=Y)) (fun x _ => (5, fun l _ => (1, fun d _ => (lookupTime (size (enc x)) l,tt)))).
  Proof.
...
Proof.
  intros H1 H2.
  induction L as [ |[a' b'] L]. now firstorder.
  cbn.
