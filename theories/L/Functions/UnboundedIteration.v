From Complexity.L.Datatypes Require Import LSum LOptions.
From Complexity.Libs Require Export LoopSum.
...
Section uiter.
  ...
  Lemma uiter_total_instanceTime {Z} `{encodable Z} (f':  Z -> Y) (preprocess : Z -> X) preprocessT (fuel : Z -> nat)
    ...
    computesTime (TyArr _ _) f' (convert (λ x, !!uiter (!!(extT preprocess) x)))
      (fun z _ => (1 + fst (preprocessT z tt) + uiterTime (fuel z) (preprocess z),tt)).
  Proof.
    cbn [convert TH "-"].
    intros total.
