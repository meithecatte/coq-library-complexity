...
From Complexity.Libs Require Import Retracts MoreList.
...
Section Encode_list.
  ...
  (* Undecidability already has a stripped down version of [encode_list], with the encoding function being
   * an explicit parameter (the [codable] class has been removed from Undecidability itself).
   * As we make use of their StepTM code, we need our encode_list to be compatible with theirs. Thus, we
   * first develop the additional lemmas with the encoding function for [X] being provided explicitly,
   * and then wrap them into an interface that makes use of the [codable] typeclass. *)
  Section FixF.
    Variable (f : X -> list sigX).

    Lemma encode_list_concat l:
      encode_list f l = concat (map (fun t => sigList_cons :: map sigList_X (f t)) l) ++[sigList_nil].
    Proof.
      induction l;cbn. reflexivity.
      rewrite IHl. cbn. now autorewrite with list.
    Qed.

    Lemma encode_list_eq_nil (xs : list X) : encode_list f xs = [sigList_nil] -> xs = nil.
    Proof. destruct xs; cbn; congruence. Qed.

    Lemma encode_list_injective : injective f -> injective (encode_list f).
    Proof.
  ...
  Notation encode_list := (@encode_list sigX X encode).
