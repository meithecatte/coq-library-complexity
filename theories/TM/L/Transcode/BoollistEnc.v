...
From Complexity.TM.L Require Alphabets.
...
Proof.
  eexists. intros bs. rewrite Code.size_list. erewrite (MoreBase.sumn_map_le_pointwise (f2:=fun _ => _)).
...
Proof.
  eexists. intros bs. rewrite Code.size_list. erewrite <- (MoreBase.sumn_map_le_pointwise (f1:=fun _ => _)).
  2:{ intros [] ?;cbv. all:reflexivity. } setoid_rewrite MoreList.sumn_map_c. instantiate (1:=2). nia.
Qed.
