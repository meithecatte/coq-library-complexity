...
From Complexity.Libs Require Import MoreFin.
From Complexity Require Import TM.Code.ProgrammingTools LM_heap_def.
From Complexity.TM.L Require Import Alphabets CaseCom LookupTM JumpTargetTM.
From Complexity.TM.Code Require Import List.App List.Length CaseList CasePair CaseSum.
...
Section StepMachine.
  ...
  Proof.
    ...
    }
    {
      ...
      {
        instantiate (1 := [| _;_;_|]).
        intros i. destruct_fin i; cbn; simpl_surject; TMSimp; isVoid_mono.
      }
      destruct ymid.
      {
    ...
    }
    {
      ...
      { (* Then of first [CaseList], i.e. [V = g :: V'] *) rename H into HCaseList, H0 into HIf'.
        ...
        destruct HIf'; TMSimp. (* This takes VERY long *)
        {
          ...
          modpon HConsClos.
          repeat split; auto.
          - intros i; destruct_fin i; cbn; auto; TMSimp; auto.
        }
  ...
  Proof.
    ...
    }
    {
      ...
      { rename H into HLookup, H1 into HCons, H3 into HReset. 
        ...
        - intros i; destruct_fin i; auto; TMSimp; cbn; rewrite !vector_tl_nth; auto.
      }
      { now modpon H. }
    }
  Qed.
    ...
    }
    {
      ...
      { (* Then of second [CaseList], i.e [P = t :: P'] *) rename H7 into HCaseList', H8 into HCaseCom, H10 into HCase.
        ...
        { (* retT *)
          ...
          { (* lamT *)
            rename HCase into HStepLam. modpon HStepLam; TMSimp; eauto; try contains_ext.
            { instantiate (1 := [| _;_;_;_;_|]).
              intros i; destruct_fin i; auto; TMSimp; cbn. all:eauto.
            }
            destruct ymid.
            - cbn. destruct HStepLam as (jump_P&jump_Q&HStepLam); modpon HStepLam.
              do 3 eexists. unfold Step_size;cbn. repeat split; eauto.
              + econstructor. eauto.
              + generalize (HStepLam3 Fin0); generalize (HStepLam3 Fin1); generalize (HStepLam3 Fin2); generalize (HStepLam3 Fin3); generalize (HStepLam3 Fin4); generalize (HStepLam3 Fin5); generalize (HStepLam3 Fin6); cbn; TMSimp; intros.
                cbn.
                destruct_fin i; TMSimp; cbn; auto; try rewrite HStepLam0 by vector_not_in; TMSimp; try rewrite !vector_tl_nth; auto.
            - cbn. split; auto. intros s' HStep. inv HStep. congruence.
          }
          { (* appT *)
            rename HCase into HStepApp. cbn in HStepApp.
            cbv [put] in *. modpon HStepApp; TMSimp; eauto; try contains_ext.
            { instantiate (1 := [| _;_;_;_;_;_|]).
              intros i; destruct_fin i; auto; TMSimp. all:cbn;eauto.
            }
            destruct ymid; cbn.
          ...
          { (* varT *)
            ...
            rename HCase into HStepVar. modpon HStepVar; TMSimp; eauto; try contains_ext.
            destruct ymid; cbn.
            - destruct HStepVar as (g&HStepVar); modpon HStepVar.
              do 3 eexists; repeat split; eauto.
              + econstructor; eauto.
              + generalize (HStepVar3 Fin0); generalize (HStepVar3 Fin1); generalize (HStepVar3 Fin2); generalize (HStepVar3 Fin3); generalize (HStepVar3 Fin4); cbn; TMSimp; intros.
                simpl_not_in. destruct_fin i; cbn; auto; TMSimp; auto.
      ...
      { (* Else of the first [CaseList], i.e. [T = nil] *)
        ...
        repeat split; eauto.
        abstract (intros i; destruct_fin i; TMSimp; cbn in *; auto).
      }
    }
  Qed.
  ...
  Proof.
    ...
    }
    {
      ...
      - hnf; cbn. do 5 eexists; repeat split; TMSimp; eauto. intros i; destruct_fin i; cbn; TMSimp; auto.
      - hnf; cbn. do 5 eexists; repeat split; TMSimp; eauto. intros i; destruct_fin i; cbn; TMSimp; auto.
      - destruct HCaseCom as (?&->&?). hnf; cbn. do 6 eexists; repeat split; TMSimp; eauto. simpl_surject. contains_ext.
    }
  Qed.
