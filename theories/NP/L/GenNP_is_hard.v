From Complexity.L.Datatypes Require Import LProd LTerm LBool.
...
From Complexity.L.Functions Require Import Size.

Import Nat.
Import L_Notations.

(* Theorem 9 in the Cook-Levin paper *)
Lemma NPhard_GenNP X__cert `{R__cert : encodable X__cert}:
  canEnumTerms X__cert -> NPhard (GenNP X__cert).
Proof.
  (* Let [enumTerm] be a polynomial-time surjection from [X__cert] to [term]
   * that in addition is non-wasteful, i.e. can produce each [c : term] from
   * an encoding as [X__cert] that is not significantly larger than [c]. *)
  intros (enumTerm&[comp1]&[comp2]&[comp3])%canEnumTerms_compPoly.
  (* Assume a problem [Q : X -> Prop] is in NP *)
  ...
  (* [f] is a function deciding whether [c] is a valid certificate for [x].
   * We can compute it in polynomial time. *)
  pose (f (x : X) (c : X__cert) :=
    f__decInTime R__comp (x,f__toTerm enumTerm c)).
  ...
  (* The reduction map. *)
  pose (g := fun (x:X) => (lam (trueOrDiverge (extT f (enc x) #0)),
                mSize (size (enc x)),
                steps (size (enc x)))).
  apply reducesPolyMO_intro_restrictBy_out with (f:= g);cbn [fst snd].
  (* Focus on the more complex correctness subgoal first, to resolve
   * the [mSize] and [steps] evars before proving that the reduction
   * is computable in polynomial time. *)
  2:{
    intros x. remember (g x) as x0 eqn:eqx0. destruct x0 as ((t0,maxSize0),steps0).
    unfold g in eqx0. injection eqx0. intros Esteps0 Esize0 Et0. clear eqx0.
    ...
    + repeat simple apply conj.
      ...
      * (* if any certificate is valid, then a small one is valid *)
        ...
        assert (eqb : f x c = true).
        { eapply trueOrDiverge_eval.
          rewrite <- Ht0''. eapply evalIn_eval_subrelation, Ht. }
        rewrite eqb in Ht0''.
        assert (H:=eqb).
      ...
      * (* on small certificates, we don't need much time *)
    ...
    + split.
      * intros (c&?&H')%(complete__pCR R__spec). cbn [proj1_sig] in H'.
